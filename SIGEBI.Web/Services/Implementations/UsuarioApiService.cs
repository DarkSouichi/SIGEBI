...
namespace SIGEBI.Web.Services
{
    public class UsuarioApiService : IUsuarioApiService
    {
        ...
        public async Task<GetAllUsuariosResponse> GetAll()
        {
            ...
            var response = new GetAllUsuariosResponse();
            ...
            try
            {
                ...
                if (httpResponse.IsSuccessStatusCode)
                {
                    response = JsonSerializer.Deserialize<GetAllUsuariosResponse>(json,
                        ...
                        ?? new GetAllUsuariosResponse { isSuccess = false, message = "Error al deserializar." };
                }
                else
                {
            ...
            try
            {
                ...
                if (httpResponse.IsSuccessStatusCode)
                {
                    response = JsonSerializer.Deserialize<GetUsuarioResponse>(json,
                        new JsonSerializerOptions { PropertyNameCaseInsensitive = true })
                        ?? new GetUsuarioResponse { isSuccess = false, message = "Error al deserializar." };
                }
                else
                {
                    var errorResponse = JsonSerializer.Deserialize<ApiResponse>(json,
